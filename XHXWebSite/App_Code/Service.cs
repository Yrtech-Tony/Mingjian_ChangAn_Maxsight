...
public class Service : System.Web.Services.WebService
{
    ...
    public byte[] SearchAnswerDtl2Pic(string picName, string shopName,string subjectCode, string type, string code)
    {
        ...
        if (!File.Exists(filePath))
        {
            ...
            try
            {
                ...
                aliyun.GetObject("yrtech", "ChangAn" + @"/" + shopName + @"/" + subjectCode + @"/" + picName.Replace(".jpg", "") + ".jpg",
    ...
    public void DeletePicture(string shopName,string fileName,string subjectCode)
    {
        ...
        ali.DeleteObject("yrtech", "ChangAn" + @"/" + shopName + @"/" + subjectCode + @"/" + fileName.Replace(".jpg", "") + ".jpg");
    }
    [WebMethod]
    ...
    #region Toyota
    [WebMethod]
    public DataSet SearchInspectionStandardResult(string projectCode, string shopCode)
    {
        string sql = string.Format("EXEC up_DSAT_AnswerDtl_All_R '{0}','{1}'", projectCode, shopCode);
        DataSet ds = CommonHandler.query(sql);
        return ds;
    }
    [WebMethod]
    public DataSet SearchLossResult(string projectCode, string shopCode)
    {
        string sql = string.Format("EXEC up_DSAT_AnswerDtl3_All_R '{0}','{1}'", projectCode, shopCode);
        DataSet ds = CommonHandler.query(sql);
        return ds;
    }
    [WebMethod]
    ...
    #region 广汽菲克获取错误分数
    [WebMethod]
    public DataSet ErrorScoreListSearch(string projectCode, string shopCode)
    {
        string sql = string.Format("EXEC up_DSAT_ErrorScore'{0}','{1}'", projectCode, shopCode);
        DataSet ds = CommonHandler.query(sql);
        return ds;
    }
    [WebMethod]
    public DataSet PadUpoadDateSearch(string projectCode)
    {

        string sql = string.Format("EXEC up_DSAT_PadUploadDateSearch'{0}'", projectCode);
        DataSet ds = CommonHandler.query(sql);
        return ds;
    }
    #endregion
    #region 下载sqllite 时先复制数据
    [WebMethod]
    public void CopyDataToSqlLite(string projectCode)
    {
        string sql = string.Format("EXEC up_Data_CopyDataFromMaster '{0}'", projectCode);
        DataSet ds = CommonHandler.query(sql);
    }
    #endregion
    #endregion
    ...
    [WebMethod]
    public void SaveShopArea(string saleSmallAreaCode, string afterSmallAreaCode, string shopCode)
    {
        string sql = string.Format("EXEC up_DSAT_ShopArea_S '{0}','{1}','{2}'",
                                   saleSmallAreaCode, afterSmallAreaCode, shopCode);
        CommonHandler.query(sql);
    }
    #endregion
    ...
    public void UpDateSubject(string projectCode, string subjectCode, int orderNO, decimal weight)
    {
        ...
        string sql = string.Format("EXEC up_DSAT_Subjects_OrderNO_U '{0}','{1}','{2}','{3}'", projectCode, subjectCode, orderNO, weight);
        CommonHandler.query(sql);
    }
    [WebMethod]
    public void SaveSubject(char statusType, string projectCode, string subjectCode, string implementation, string checkPoint,
                            ...
                            string remark, int orderNO, string linkCode, decimal fullScore, bool? scoreCheck, string subjectTypeCode, string subjectTypeCodeExam, bool subjectDelChk, bool addErrorChk,decimal? weight)
    {
        string sql = String.Empty;
        if (statusType == 'I' || statusType == 'U')
        {
    ...
    [WebMethod]
    public void UpDateAreaPassword(string areaCode, string userId, string password)
    {
        string sql = string.Format("EXEC [up_AreaTool_UpdateAreaUserIdAndPassword_U] '{0}','{1}','{2}'",
                             areaCode, userId, password);
        CommonHandler.query(sql);
    }
    ...
    public DataSet[] GetShopReportDto(string projectCode, string shopCode)
    {
        ...
        DataSet ds = CommonHandler.query(sql);
        DataSet[] dataSetList = new DataSet[] { ds,SearchSubjectsScore(projectCode,shopCode)};
  ...
  @SpecialCaseCode = '{0}'", specialCaseCode);
    ...
    public void Area_SubjectSave(string projectCode, string subejctCode, string checkItem, string evaluationStandard, int executionNumber, decimal? weight, string linkCode, string userId, string subjectCode1)
    {
        string sql = string.Format("EXEC [GACFCA_AreaTool].[dbo].up_Surveys_S '{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}'",
            projectCode, subejctCode, checkItem, evaluationStandard, executionNumber, weight, linkCode, userId, subjectCode1);
        DataSet ds = CommonHandler.query(sql);
    }
    [WebMethod]
    ...
    {
        string sql = string.Format("EXEC [GACFCA_AreaTool].[dbo].up_Link_S '{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}'", projectCode, linkCode, linkName, weight, bussinessTypeCode, userid, surveyTypeCode, linkCode1);
        DataSet ds = CommonHandler.query(sql);
    }
}
