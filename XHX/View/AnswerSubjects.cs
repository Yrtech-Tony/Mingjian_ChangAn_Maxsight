...
namespace XHX.View
{
    ...
    public partial class AnswerSubjects : BaseForm
    {
        ...
        public AnswerSubjects()
        {
            ...
            CommonHandler.SetRowNumberIndicator(grvFileAndPic);
            CommonHandler.SetRowNumberIndicator(grvInspectionStandard);
            CommonHandler.SetRowNumberIndicator(grvLoss);
        }
        ...
        private void btnShopCode_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            {
                shop.ExamTypeCode = ds.Tables[0].Rows[0]["SubjectTypeCodeExam"] == null ? "A" : ds.Tables[0].Rows[0]["SubjectTypeCodeExam"].ToString();
            }
            else
            {
                shop.ExamTypeCode = "A";
        ...
        private void btnNext_Click(object sender, EventArgs e)
        {
            ...
            {
                service.SaveLossDesc(ProjectCode_Golbal, ShopCode_Golbal, SubjectCode_Golbal, dto.LossName, dto.PicName, dto.SeqNO, dto.StatusType);
            }
            //查询下一个问卷信息并显示
        ...
        public bool GetSubject(DataSet ds, ButtonType buttonType)
        {
            ...
            if (subjectDto.LastProjectScore == 9999)
            {
                ...
                txtLastScore.Text = string.Empty;
            }
            else
            {
                chkLastNotinvolved.Checked = false;
        ...
        private void btnPictureUpload_ButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            if (uploaddto.FilePath.Contains(':') && uploaddto.FilePath.Contains('\\'))
            {
                ...
                //    image = new byte[0];
                //}
                aliyun.PutObject("yrtech", "ChangAn" + @"/" + CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + txtShopName.Text + @"/" + SubjectCode_Golbal + @"/" + uploaddto.FileName + ".jpg", filePath);
        ...
        private void grcLoss_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (bdhi.InRow)
            {
                ...
                for (int i = 0; i < filenameList.Length; i++)
                {
                    ...
                    //    image = new byte[0];
                    //}
                    aliyun.PutObject("yrtech", "ChangAn" + @"/" + CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + txtShopName.Text + @"/" + SubjectCode_Golbal + @"/" + filenameList[i] + ".jpg", filePath);
        ...
        private void btnRecheckApply_Click(object sender, EventArgs e)
        {
            ...
            {
                CommonHandler.ShowMessage(MessageType.Information, "没事有设置试卷类型");
                return;
            }
            if (ds.Tables[0].Rows.Count > 0)
            {
        ...
        private void btnUploadDataToServer_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    //Makefolder((CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + txtShopName.Text), "ftp://123.57.229.128/UpLoadImage/", "dsat", "dsat2011");
                    ...
                    //    DeleteFTPFile(url,
                    //        ("ftp://123.57.229.128/UpLoadImage" + "/" + (CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + txtShopName.Text+"/")).Replace("\\", "//"), "dsat", "dsat2011");
                    //}
                    foreach (string name in AllFilesList)
                    {
                        if (!name.Contains("Thumbs.db"))
                        {
                            ...
                            string url = temp[temp.Length - 1];
                            //DeleteFTPFile(url,
                            //    ("ftp://123.57.229.128/UpLoadImage" + "/" +
                            //    (CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + txtShopName.Text+@"/")).Replace("\\", "//"), "dsat", "dsat2011");
                            //Upload(name,
                            //    ("ftp://123.57.229.128/UpLoadImage" + "/" + (CommonHandler.GetComboBoxSelectedValue(cboProjects).ToString() + txtShopName.Text) + "/" + url).Replace("\\", "//"), "dsat", "dsat2011");
